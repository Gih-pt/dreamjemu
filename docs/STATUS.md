...
*Last updated: 2026-07-26 (core-gdrom: CDI sector reading implemented, fixing a real detector bug along the way; IP.BIN boot header parsing added as the first HLE boot sketch step; new app-cli module for GUI-free disc image testing; core-cpu-sh4: added post-increment/pre-decrement/indexed/displacement/PC-relative addressing modes). Update this file whenever a contribution meaningfully changes what's implemented — see `CONTRIBUTING.md`.*
...
- [ ] SH-4: the rest of the instruction set (logic/shift/subroutine calls/JMP/byte-word memory access/multiply/divide/most addressing modes now covered — post-increment, pre-decrement, R0-indexed, 4-bit displacement, and PC-relative are all done; still missing: `RTE` — needs SSR/SPC and exception-mode state — MMU, caches, exceptions/interrupts, and the GBR-register-based addressing modes (`@(disp,GBR)`, `@(R0,GBR)` — GBR itself isn't modeled yet)), delay slots are handled for all currently-implemented delayed branches (`BRA`/`BSR`/`JSR`/`JMP`/`RTS`).
